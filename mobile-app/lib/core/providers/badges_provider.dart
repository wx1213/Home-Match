...
///
/// "我需要做的事"必须是**当前用户视角**：
/// - "我还没评价" 而不是 "对方还没评价"（避免对方没评就显示"我有 1 个待办"）
/// - 旁观者（既不是 buyer 也不是 seller）不计数
final tabBadgesProvider = FutureProvider.autoDispose<TabBadges>((ref) async {
  // 订阅 userId 变化：切换身份时这个 provider 会被销毁重建
  final myId = ref.watch(authProvider.select((a) => a.userId));
  ...
  //   ⚠️ 必须是"我"没评，不是"对方"没评（详见 tabBadgesProvider 注释）
